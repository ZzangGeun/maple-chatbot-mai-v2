...
로컬 및 개발 서버 구동을 위해 다음 5가지 서비스가 컨테이너로 연동됩니다.
* **web (Django Backend):** RDB 마이그레이션 관리, 어드민, 사용자 인증 담당
* **ai-server (FastAPI):** 비동기 챗봇 API, RAG 질의, Nexon Open API 호출 담당
* **db (PostgreSQL):** 정적 및 사용자 데이터 저장
* **redis (Cache & Lock):** 넥슨 Open API 데이터 캐시 및 호출 Rate Limit 기록
* **chroma-db (Vector Store):** 가이드 및 패치 정보 문서 벡터 검색
